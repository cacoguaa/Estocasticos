...
public class Listener extends Python3BaseListener {
	...
	public void enterAnd_test(Python3Parser.And_testContext ctx) {
		double prob = probModule.addDerivation(ctx.getRuleIndex(),
				ctx.invokingState);
	...
	public void enterFile_input(Python3Parser.File_inputContext ctx) {
		...
		if (bCalculate){
			System.out.println(bCalculate + " bcal");
	...
	private double calculateChildProb(double probFather, RuleContext parseTree) {
		// TODO Auto-generated method stub
		// FIXME PUTO
		double probchil = probModule.probDerivation(parseTree.getRuleIndex(),parseTree.invokingState);
		...
		for (int i = 0; i < parseTree.getChildCount(); i++) {
			if (!parseTree.getChild(i).getClass()
				...
				probchil *= probchil * calculateChildProb(probchil, child);
			}
			// TODO Creo que esta mal
		}
		acumulate += probchil;
		System.out.println(probchil + " Prob child");
		return probchil;
	}
